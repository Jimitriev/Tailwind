...
type propsType = {
    label?: String,
    className?: String,
    type?: currencyTypeInterface,
    setType?: (e: currencyTypeInterface) => void
    amount?: number,
    setAmount?: (e?: any) => void,
    balance?: number,
    typeOut?: currencyTypeInterface
}

const CurrentSelector = (props: propsType) => {
    ...
    const changeTypeHandle = (newType: currencyTypeInterface) => {
                ...
                toggle ?
                    <div className='currencySelectorDropDownMenu w-full z-20 absolute left-0 top-[80px] bg-[#201B55] p-1'>
                        ...
                        <div className='w-full overflow-x-hidden overflow-y-auto max-h-[250px] scrollbar-w-[4px] scroll-smooth scrollbar-thin scrollbar-thumb-primary scrollbar-track-transparent scrollbar-corner-transparent scrollbar-thumb-rounded-full scrollbar-track-p-2'>
                            {
                                currencyTypes.filter((item: currencyTypeInterface) => {
                                ...
                                }).map((item: currencyTypeInterface, index) => {
                                    return (
                                        <div key={index + "_currency"} className={`w-full flex items-center justify-between cursor-pointer transition-colors hover:bg-primary px-3 py-3 ${currencyTypes.length - 1 != index ? 'border-b border-[#3B3B77]' : ''}`}
